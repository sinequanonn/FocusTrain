  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <div className="order-2 lg:order-1">
        ...
        <div className="mt-4 flex items-center justify-between rounded-2xl border border-gray-100 bg-white px-5 py-3 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-gray-700">
              {departure.name} <span className="text-gray-300">→</span>{' '}
              {arrival.name}
            </span>
          </div>
      ...
      <aside className="order-1 space-y-4 lg:order-2">
        <section className="rounded-3xl border border-[#2AC1BC]/30 bg-white p-6 shadow-sm">
          {/* 타이머 모드 토글 */}
          <div className="mb-3 inline-flex rounded-full border border-gray-200 bg-gray-50 p-1 text-[11px] font-bold">
            <button
              onClick={() => setTimerMode('remaining')}
              className={`rounded-full px-3 py-1 transition ${
                timerMode === 'remaining'
                  ? 'bg-white text-[#2AC1BC] shadow'
          ...
          <div className="space-y-2">
            {isRunning ? (
              <button
                onClick={onPause}
                disabled={busy}
                className="w-full rounded-2xl border border-gray-200 bg-white py-3 font-bold text-gray-700 hover:bg-gray-50 disabled:opacity-50"
              >
                ⏸ 하차 (일시정지)
              </button>
            ) : (
            ...
            <button
              ...
              className="w-full rounded-2xl bg-[#2AC1BC] py-3 font-bold text-white hover:opacity-90 disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400"
              title={
                isRunning
                  ? '하차(일시정지) 후 완료할 수 있어요'
                  : targetReached
            ...
            <button
              ...
              className="w-full rounded-2xl border border-red-200 bg-red-50 py-3 font-bold text-red-500 hover:bg-red-100 disabled:cursor-not-allowed disabled:border-gray-200 disabled:bg-gray-100 disabled:text-gray-300"
